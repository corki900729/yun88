...
//         }
       ...
       // dump($userid);exit();
        if ($userid > 0) {
            ...
            cookie("forward",$_REQUEST['forward']);
   ...
   * 验证短信在24小时内发送的次数
   * */
  public function checkSms($cellPhone){
      if(empty(cache($cellPhone))){
          $i = 1;
          cache($cellPhone,$i,10);
      }else{
          $i = cache($cellPhone)+1;
          cache($cellPhone,$i);
      }

      $times = cache($cellPhone);
        if($times<3){
            return false;
        }else{
            return true;
    ...
    public function dosmsCode() {
        ...
        $cellPhone = I('request.cellPhone', '');
        $code = I('request.code');
        $token = I('request.token');
        if(empty($cellPhone) || !preg_match("/^1[34578][0-9]{9}$/", $cellPhone)){
            $this->error('手机号不能为空');
        }
        $times = cache($cellPhone);
        if($this->checkSms($cellPhone)){
            $this->ajaxReturn(array(
                'status'=>0,
                'info'=>'不能连续发送3次'
            ));
        }
        $info = session('smsCode_'.$cellPhone);
        if($info['mobile'] == $cellPhone && $info['ctime'] >= time()-600){
            $captcha = $info['captcha'];
        }else{
            $captcha = rand(100000, 999999);
        }
        if(session('username'.$this->userid)!=$token){
            if (!$this->verify($code)) {
                $this->ajaxReturn(array('info'=>"操作错误",'status'=>0));
            }
        }

        $content = $this->sendSMS($cellPhone,'尊敬的用户，您本次短信验证码为：'.$captcha.'。10分钟内有效，请注意查收，勿向他人泄露此验证码【yun88】');
//        echo json_encode($content);
        if($content){
            if ($content['result'] == '0') {
                $result = array('mobile'=>$cellPhone, 'captcha'=>$captcha, 'ctime'=>time()+12000);
                session('smsCode_'.$cellPhone, $result);
                unset($result);
                //$this->error('短信验证码发送成功');
                $data = array('info'=>'短信验证码已发送,注意查收','status'=>1,'mes'=>$times);
            } else {
                session('smsCode_'.$cellPhone, null);
            }

        }else{
            record($content,'smscode');
            $data = array('info'=>$content,'status'=>0);
   ...
   private function sendSMS($mobile, $content){
        $useranme = 'bjydsd';
        $passord = '123456';
        $send_sms_url = 'http://www.sms1086.com/plan/api/Send.aspx?username='.$useranme.'&password='.$passord.'&mobiles='.$mobile.'&content=';
        $send_content = urlencode(iconv('UTF-8', 'GBK', $content));
        $fp = fopen($send_sms_url.$send_content, 'r');
        $ret = fgetss($fp, 255);
        $smd_result = array();
        parse_str(iconv('GBK', 'UTF-8',urldecode($ret)), $smd_result);
        fclose($fp);
        return $smd_result;
    }
    ...
    public function doRegister() {
        ...
        if(!isMax($post['username'],20)){
            $this->error('用户名长度不能超过20个字符');
        }
        //邮箱
        $post['email'] = I('post.remail');
//         if (empty($post['email'])) {
//             $this->error('邮箱不能为空！');
//         }
        ...
        //设置密码
        $post['password'] = I('post.rpassword');
        ...
        //确认密码
        $password2 = I('post.rpassword2');
        if ($post['password'] != $password2) {
            $this->error('两次输入密码不相同！');
        }

        $post['mobile'] = I('post.cellPhone','');
         if(empty($cellPhone) || !preg_match("/^1[34578][0-9]{9}$/", $cellPhone)){
             $this->error('手机号不能为空');
         }
       ...
       $smsCode = I('post.cap');
        ...
        $smsInfo = session('smsCode_'.$post['mobile']);
        if($smsInfo['ctime'] < time()-600) {
            session('smsCode_'.$post['mobile'], null);
            $this->error('短信验证码过期，请重新获取！');
        }
        if($smsCode != $smsInfo['captcha']) {
            $this->error('短信验证码错误！');
        }
                    ...
                    if ($this->memberConfig['enablemailcheck']) {
                        ...
                        if (empty($message)) {
                            $message = 'Hi，{$username}:
                                                    ...
                                                    欢迎您注册成为 YUN88 用户，您的账号需要邮箱认证，点击下面链接进行认证：
