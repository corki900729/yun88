...
class IndexController extends Base {
    ...
    public function index() {
		...
		//首页获取
		$cloudlist = M('Company')->field('name,com_id,index_logo,txt_intro,img_logo')->where("type=1 and is_show=1 and index_logo <> ''")->select();
		$applist = M('Company')->field('name,com_id,index_logo,txt_intro')->where("type=2 and is_show=1 and index_logo <> ''")->select();
...
//        $this->assign('cate',$cate);
		...
		$this->assign('cloud',$cloudlist);
		$this->assign('app',$applist);
        $this->display("Index:" . $tp[0]);
    }
    public function search(){
        $key = I('request.t');
        $data = M('GoodsPackage')
            ->where("name LIKE '%$key%' OR instro LIKE '%$key%'")
            ->select();
        $data4 = M('Goods')
            ->where("name LIKE '%$key%' ")
            ->select();
...
//        dump($template);die;
    ...
    public function shows() {
        ...
        $this->assign('title',$output_data['title']);
        $this->assign("img_logo", $output_data['img_logo']);
